...
public class ComponentUtil extends AndroidNonvisibleComponent {
    ...
    public void RequestFocus(final AndroidViewComponent component) {
        View view = component.getView();
        boolean check = view.requestFocus();
        AfterFocus(component, check);
    }

    @SimpleEvent(description = "This event is fired after the extension has attempted to give focus to a component.")
    public void AfterFocus(AndroidViewComponent component, boolean successful) {
        EventDispatcher.dispatchEvent(this, "AfterFocus", component, successful);
    }

    @SimpleFunction(description = "Scrolls the scrollable arrangement to the given position.")
    public void ScrollTo(AndroidViewComponent scrollableArrangement, int x, int y) {
        View view = scrollableArrangement.getView();
        view.scrollTo(x, y);
    }
    ...
    public void SetLongClickable(AndroidViewComponent component, boolean longClickable) {
        View view = component.getView();
        view.setLongClickable(longClickable);
    }

    @SimpleFunction(description = "Registers the component so that when a hardware key is pressed in this component, " + 
    "the extension will fire the OnKey event.")
    public void RegisterOnKey(final AndroidViewComponent component) {
        View view = component.getView();
        view.setOnKeyListener(new View.OnKeyListener() {
            @Override
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                OnKey(component, keyCode);
                return false;
            }
        });
    }
    ...
    public void OnKey(AndroidViewComponent component, int keyCode) {
        EventDispatcher.dispatchEvent(this, "OnKey", component, keyCode);
    }

    @SimpleFunction(description = "Unegisters the component so that when a hardware key is pressed in this component, " + 
    "the extension will not fire the OnKey event.")
    public void UnregisterOnKey(final AndroidViewComponent component) {
        View view = component.getView();
        view.setOnKeyListener(null);
    }

    // new features added in version 4

    @SimpleFunction(description = "Moves the scrolled position of your scrollable arrangement.")
    public void ScrollBy(AndroidViewComponent scrollableArrangement, int x, int y) {
        View view = scrollableArrangement.getView();
        view.scrollBy(x, y);
    }

    @SimpleFunction(description = "Clears any pivot previously set by a call to SetPivotX() or SetPivotY()." + 
    " After calling this IsPivotSet() will be false and the pivot used for rotation will return to default of being centered on the component.")
    public void ResetPivot(AndroidViewComponent component) {
        View view = component.getView();
        view.resetPivot();
    }

    @SimpleFunction(description = "Checks if the pivot was set for this component.")
    public boolean IsPivotSet(AndroidViewComponent component) {
        View view = component.getView();
        return view.isPivotSet();
    }

    // new features added in version 5

    @SimpleFunction(description = "Applies ripple effect for the given component")
    public void SetRipple(AndroidViewComponent component, int color){
    ...
    public void SetScrollbarFadingEnabled(AndroidViewComponent component, boolean fade) {
        View view = component.getView();
        view.setScrollbarFadingEnabled(fade);
    }

    @SimpleFunction(description = "Checks whether the component can draw on its own.")
    public boolean WillNotDraw(AndroidViewComponent component) {
        View view = component.getView();
